---
title: "GraphRAG: 拆解neo4j-text2sql"
date: 2026-06-25
---
...
- 标准的Text-to-SQL实现为LLM提供完整的数据库schema，会导致几个问题：
    1. Token使用多，价格贵速度慢
    2. 上下文噪声，导致幻觉和准确性下降
- 为了解决这些问题，`neo4j-text2sql`使用Neo4j作为动态语义层，引入GraphRAG。
...
# 个人调试中的改造/问题
- 使用docker-compose容器化postgres, neo4j服务
- 我只有GPT plus账号，不想开openai账号，做了如下改造
  - embedding模型，使用本地V100显卡serve的`BAAI/bge-m3`
  - chat模型，使用本地`omniroute`
- 修改chat模型后，会导致`langchain_openai.chat_models.base#apply_langchain_openai_compat`中`normalized_token_usage`出现异常，异常会被吞
- `semanticLayerTool`中agent_embedding错误

# What's next?
- PostgreSQL在国内用得比较少吧，项目中用到了一些PostgreSQL的专属特性；而MySQL, StarRocks等更适合中国宝宝体质。设计图schema时需要尽可能做成通用的，并且需要建立一层db adapter抽象来摄取图数据。
- `business_glossary.json`需要提前整理好。语义层可以做成compounding的：Agent在被使用中理解了被澄清的语义并融入Neo4j语义层。
- `neo4j-text2sql`借鉴了`neocarta`的做法，下一篇展开讲`neocarta`并进行对比。
